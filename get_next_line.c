...
/*   Updated: 2019/12/04 17:28:31 by arraji           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	while (!is_line(save, rd))
	{
		...
		if (!save)
			return (ft_end(&buff, NULL, -1));
		if (!rd && !ft_strlen(save, 1))
			return (ft_end(&save, &buff, 0));
		if (ft_stradd(&save, buff, 1) == -1)
			return (ft_end(&buff, &save, -1));
	}
	return (ft_end(&buff, NULL, cutter(&save, line)));
}
